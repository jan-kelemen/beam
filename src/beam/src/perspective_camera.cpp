...
beam::perspective_camera::perspective_camera() : extent_{1920, 1080} { }

glm::uvec2 const& beam::perspective_camera::extent() const { return extent_; }

void beam::perspective_camera::resize(glm::uvec2 const& extent)
{
    extent_ = extent;
    aspect_ratio_ = cppext::as_fp(extent.x) / cppext::as_fp(extent.y);
}

std::pair<glm::vec3, glm::vec3> beam::perspective_camera::raycast(
